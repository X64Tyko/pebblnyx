...
// PNX_USE_BITPLANE_COMPRESS viable against a renderer with no dirty-tracking (see
// pnx_config.h's PNX_TILE_CACHE_SLOTS comment). Standalone, like pnx_bitplane.c itself:
// not wired into pnx_map_load/pnx_tilemap_draw_layer yet, so this is the only place it
// runs, against a mock fetch callback rather than a real resource file (the on-disk
// per-tile-addressable atlas format this would eventually read from doesn't exist yet).
//
...
void test_tile_cache(void)
{
	...
	const uint8_t* p1 = pnx_tile_cache_get(1, 0, mock_fetch, NULL);
	TC_CHECK(p1 != NULL);
	...
	const uint8_t* p1_again = pnx_tile_cache_get(1, 0, mock_fetch, NULL);
	...
	const uint8_t* p2 = pnx_tile_cache_get(1, 1, mock_fetch, NULL);
	...
	const uint8_t* p3 = pnx_tile_cache_get(2, 0, mock_fetch, NULL);
	TC_CHECK(p3 != NULL);
	...
	for (uint16_t i = 0; i < PNX_TILE_CACHE_SLOTS; i++)
		TC_CHECK(pnx_tile_cache_get(100, i, mock_fetch, NULL) != NULL);
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS);
	...
	for (uint16_t i = 0; i < PNX_TILE_CACHE_SLOTS; i++)
		pnx_tile_cache_get(100, i, mock_fetch, NULL);
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS); // still no new fetches
	...
	TC_CHECK(pnx_tile_cache_get(100, PNX_TILE_CACHE_SLOTS, mock_fetch, NULL) != NULL);
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS + 1);
	pnx_tile_cache_get(100, 0, mock_fetch, NULL); // now a miss again -- it was evicted
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS + 2);

	// --- touching a slot protects it from being the next eviction victim ---
	pnx_tile_cache_reset();
	s_fetch_calls = 0;
	for (uint16_t i = 0; i < PNX_TILE_CACHE_SLOTS; i++)
		pnx_tile_cache_get(200, i, mock_fetch, NULL);
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS);
	// Age every slot by one tick, then re-touch key 0 (resets ITS age to 0, so key 1 is
	// now the oldest, not key 0).
	pnx_tile_cache_tick();
	pnx_tile_cache_get(200, 0, mock_fetch, NULL);
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS);				 // still a hit, no new fetch
	pnx_tile_cache_get(200, PNX_TILE_CACHE_SLOTS, mock_fetch, NULL); // forces one eviction
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS + 1);
	pnx_tile_cache_get(200, 0, mock_fetch, NULL); // key 0 was just touched -- must survive
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS + 1);
	pnx_tile_cache_get(200, 1, mock_fetch, NULL); // key 1 was the real oldest -- must be gone
	TC_CHECK(s_fetch_calls == PNX_TILE_CACHE_SLOTS + 2);
	...
	pnx_tile_cache_reset();
	s_fetch_calls = 0;
	TC_CHECK(pnx_tile_cache_get(300, 0, mock_fetch, NULL) != NULL);
	TC_CHECK(s_fetch_calls == 1);
	for (uint32_t t = 0; t < PNX_TILE_CACHE_MAX_AGE; t++)
		pnx_tile_cache_tick();
	pnx_tile_cache_get(300, 0, mock_fetch, NULL);
	TC_CHECK(s_fetch_calls == 2); // released and refetched

	// --- reset clears everything unconditionally ---
	pnx_tile_cache_get(400, 0, mock_fetch, NULL);
	...
	pnx_tile_cache_reset();
	s_fetch_calls		= 0;
	const uint8_t* good = pnx_tile_cache_get(500, 0, mock_fetch, NULL);
	TC_CHECK(good != NULL);
	uint8_t good_snapshot[128];
	memcpy(good_snapshot, good, sizeof(good_snapshot));

	const uint8_t* bad = pnx_tile_cache_get(500, 1, bad_fetch, NULL);
	...
	const uint8_t* good_again = pnx_tile_cache_get(500, 0, mock_fetch, NULL);

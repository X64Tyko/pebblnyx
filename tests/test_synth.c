...
// NOLINTNEXTLINE(readability-identifier-naming) -- reads as a call, not a constant
#define note(...)            \
	do                       \
	{                        \
		printf(__VA_ARGS__); \
...
static uint32_t cost_of(const PnxSynthConfig* cfg, uint32_t chunks)
{
	...
	for (int i = 0; i < PNX_SYNTH_SLOTS; i++)
		pnx_synth_note_on((uint8_t)i, 48 + i * 5, 200);
	PnxSynthBench b;
	...
	if (b.checksum == 0x7FFFFFFF)
		note("  (impossible checksum, kept for the optimiser)");
...
static void test_record_roundtrip(void)
{
	...
	SY_CHECK("record: cutoff base/resonance/env",
			 in.cutoff_base == 60 && in.resonance == 190 && in.cutoff_env_amount == 180);
	SY_CHECK("record: lfo",
	...
	for (int i = 0; i < 3; i++)
	{
		if (in.osc[i].wave != waves[i])
			oscs_ok = false;
		if (in.osc[i].volume != (uint8_t)(200 - i * 10))
			oscs_ok = false;
		if (in.osc[i].detune != detunes[i])
			oscs_ok = false;
		if (in.osc[i].octave != octaves[i])
			oscs_ok = false;
		if (in.osc[i].duty != (uint8_t)(128 - i * 32))
			oscs_ok = false;
	}
...
static void test_bandlimited_tables(void)
{
	...
	SY_CHECK("a high note is no rougher than its pitch accounts for",
			 rough[1] <= rough[0] * 12);
	note("  saw slew/amplitude: C3 %d, C6 %d (pitch alone accounts for 8x)", (int)rough[0],
		 (int)rough[1]);
	pnx_synth_all_off();
}
...
void test_synth(void)
{
	...
	note("  heap: %u B total, %u B of that effect delay lines", (unsigned)pnx_synth_bytes(),
		 (unsigned)pnx_synth_effect_bytes());
	...
	for (int i = 0; i < PNX_SYNTH_SLOTS; i++)
		pnx_synth_set_instrument((uint8_t)i, &lead);
	...
	for (int i = 0; i < PNX_SYNTH_SLOTS; i++)
		pnx_synth_note_on((uint8_t)i, 48 + i * 5, 220);
	SY_CHECK("every slot sounds together", pnx_synth_active_voices() >= PNX_SYNTH_SLOTS);
	render_into(buf, CHUNK);
	...
	note("  kick zero crossings: %u early -> %u late (still sounding)", (unsigned)early,
		 (unsigned)late);

...
static const char *WT_FILES[] = PNX_ASSET_FILE_TABLE;
static char WT_PATHS[PNX_ASSET_COUNT][64];
...
static bool register_worldtiles(void) {
  ...
  for (uint32_t i = 0; i < PNX_ASSET_COUNT; i++) {
    RESOURCES[i] = i + 1;
    snprintf(WT_PATHS[i], sizeof(WT_PATHS[i]), "%s%s", WT_DIR, WT_FILES[i]);
    FILE *f = fopen(WT_PATHS[i], "rb");
    if (!f) {
      printf("  SKIP stream: %s not built -- run examples/worldtiles/generate.py "
             "then the pipeline\n", WT_PATHS[i]);
      return false;
    }
    fclose(f);
    pnx_host_register_resource(RESOURCES[i], WT_PATHS[i]);
  }
  return true;
}
...
static int32_t first_hole(const PnxMap *m, const PnxCamera *cam) {
  ...
  for (int32_t ty = y0; ty <= y1; ty++) {
    ...
    for (int32_t tx = x0; tx <= x1; tx++) {
      ...
      if (pnx_map_tile(m, tx, ty) == PNX_MAP_NO_CELL) return ty * 1000 + tx;
    }
  }
  return -1;
}
...
static void walk_everything(PnxMap *m, PnxCamera *cam, int32_t step,
                            int32_t *out_holes, uint32_t *out_worst_missing) {
  int32_t holes = 0;
  uint32_t worst = 0;
...
void test_stream(void) {
  ...
  S_CHECK(pnx_assets_init(&persistent, &scene, RESOURCES, PNX_ASSET_COUNT));
  ...
  S_CHECK(pnx_map_tile(&field, 0, 0) == PNX_MAP_NO_CELL);
  {
    ...
    for (int16_t y = 0; y < 228; y++) {
      ...
      for (int16_t x = 0; row.data && x < 200; x++) if (row.data[x] != 0x40) dirty++;
    }
    S_CHECK_EQ(dirty, 0);
  }
  ...
  S_CHECK(pnx_platform_resource_size(RESOURCES[PNX_ASSET_MAP_FIELD], &map_res_bytes));
  S_CHECK(map_res_bytes < 4096);
  S_CHECK(field.first_bank_asset > PNX_ASSET_MAP_FIELD);

  const uint16_t banks = (uint16_t)((((field.wt_cols * field.wt_rows) - 1)
                                     >> field.bank_shift) + 1);
  size_t worst_bank = 0;
  for (uint16_t b = 0; b < banks; b++) {
    size_t sz = 0;
    S_CHECK(pnx_platform_resource_size(RESOURCES[field.first_bank_asset + b], &sz));
    if (sz > worst_bank) worst_bank = sz;
  }
  ...
  printf("  ... %u banks, largest %u B (the plane is %u B)\n",
         banks, (unsigned)worst_bank, MAP_FIELD_W * MAP_FIELD_H * 2);
  ...
  printf("  ... walked %dx%d, worst backlog %u WorldTiles, no holes\n",
         MAP_FIELD_W, MAP_FIELD_H, worst_missing);
  ...
  printf("  ... 192x192 world: %u B streamed, %u B held whole (%ux)\n",
         (unsigned)field_bytes, (unsigned)plain_bytes,
         (unsigned)(plain_bytes / (field_bytes ? field_bytes : 1)));

...
// Fixture generated by the real encoder (tools/pnx_assets.py's lzss_compress) against
// cell-plane-shaped data -- a short run, a switch, a longer run, a back-reference into
// it, then a literal tail -- so this pins the C decoder against what the pipeline will
// actually hand it, not a hand-picked byte sequence that happens to decode.
static const uint8_t LZSS_ORIGINAL[41] = {
	0x07,
	0x00,
	0x07,
	0x00,
...
static const uint8_t LZSS_PACKED[20] = {
	0xCB,
	0x07,
	0x00,
	0x01,
...
static void test_lzss_decode(void)
{
	uint8_t out[sizeof(LZSS_ORIGINAL)];
	memset(out, 0xAA, sizeof(out));
	const size_t n = pnx_lzss_decode(LZSS_PACKED, sizeof(LZSS_PACKED), out, sizeof(out));
	A_CHECK_EQ(n, sizeof(LZSS_ORIGINAL));
	A_CHECK(memcmp(out, LZSS_ORIGINAL, sizeof(LZSS_ORIGINAL)) == 0);

	// A dst buffer shorter than the real output must stop exactly at its own length, not
	// overrun it -- the bound worldtile_load_run's own scratch sizing depends on.
	uint8_t small[10];
	const size_t n2 = pnx_lzss_decode(LZSS_PACKED, sizeof(LZSS_PACKED), small, sizeof(small));
	A_CHECK_EQ(n2, sizeof(small));
	A_CHECK(memcmp(small, LZSS_ORIGINAL, sizeof(small)) == 0);

	// A truncated/corrupt source must return early rather than read or write past what it
	// was given -- the same "short read" posture load_blob_into already takes.
	uint8_t partial_out[sizeof(LZSS_ORIGINAL)];
	memset(partial_out, 0xAA, sizeof(partial_out));
	const size_t n3 = pnx_lzss_decode(LZSS_PACKED, 3, partial_out, sizeof(partial_out));
	A_CHECK(n3 < sizeof(LZSS_ORIGINAL));
}
...
void test_assets(void)
{
	...
	// The cell dictionary (M12): real content, not a synthetic fixture, so this is the
	// proof dedup actually ran rather than merely compiled. A real map never approaches
	// 256 distinct entries (see PNX_BLOB_VERSION's v13 comment for the measured numbers),

...
static void a_enter(void *ctx)   { (void)ctx; logged("a.enter"); }
static void a_exit(void *ctx)    { (void)ctx; logged("a.exit"); }
static void a_suspend(void *ctx) { (void)ctx; logged("a.suspend"); }
static void a_resume(void *ctx)  { (void)ctx; logged("a.resume"); }
static void a_tick(void *ctx)    { ((Ctx *)ctx)->ticks++; logged("a.tick"); }
static void a_draw(void *ctx, PnxTarget *t) { (void)ctx; (void)t; logged("a.draw"); }
static void a_frame(void *ctx)   { (void)ctx; logged("a.frame"); }

static const PnxAppOps A_OPS = {
  .enter = a_enter, .exit = a_exit, .suspend = a_suspend, .resume = a_resume,
  .tick = a_tick, .draw = a_draw, .frame = a_frame,
};
...
static void b_enter(void *ctx)   { (void)ctx; logged("b.enter"); }
static void b_exit(void *ctx)    { (void)ctx; logged("b.exit"); }
static void b_suspend(void *ctx) { (void)ctx; logged("b.suspend"); }
static void b_resume(void *ctx)  { (void)ctx; logged("b.resume"); }

static const PnxAppOps B_OPS = {
  .enter = b_enter, .exit = b_exit, .suspend = b_suspend, .resume = b_resume,
};
...
void test_app(void) {
  ...
  memset(&ctx, 0, sizeof(ctx));
  pnx_app_init(&ctx);
  for (int i = 0; i < PNX_APP_MAX_DEPTH; i++) pnx_app_push(&B_OPS);

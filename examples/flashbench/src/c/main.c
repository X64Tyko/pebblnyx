...
#define READ_LEN 256u   // fixed throughout, matching PNX_PERSIST_KEY_BYTES scale, so
                        // length is never a variable this grid has to account for
...
static uint8_t point_size(uint8_t p) { return (uint8_t)(p / OFFSET_KIND_COUNT); }
static uint8_t point_kind(uint8_t p) { return (uint8_t)(p % OFFSET_KIND_COUNT); }
...
static void start_bench(App *a) {
  ...
  pnx_diag_flush();
  pnx_log("flashbench: run started -- %u sizes x %u offsets, %uB reads",
          (unsigned)SIZE_COUNT, (unsigned)OFFSET_KIND_COUNT, (unsigned)READ_LEN);
}
...
static void report_results(App *a) {
  for (int s = 0; s < SIZE_COUNT; s++) {
    ...
    pnx_format(a->result[s], sizeof(a->result[s]), "%s: near=%u deep=%u",
               SIZE_LABEL[s], (unsigned)near_us, (unsigned)deep_us);
  ...
  const uint32_t near_large = per_call_us(a->ms[(SIZE_COUNT - 1) * 2],
                                          a->calls[(SIZE_COUNT - 1) * 2]);
  const uint32_t deep_small = per_call_us(a->ms[1], a->calls[1]);
  const uint32_t deep_large = per_call_us(a->ms[(SIZE_COUNT - 1) * 2 + 1],
                                          a->calls[(SIZE_COUNT - 1) * 2 + 1]);
  pnx_format(a->summary1, sizeof(a->summary1), "near: 8K=%u 220K=%u",
             (unsigned)near_small, (unsigned)near_large);
  pnx_format(a->summary2, sizeof(a->summary2), "deep: 8K=%u 220K=%u",
             (unsigned)deep_small, (unsigned)deep_large);
...
static void frame(void *ctx, uint32_t elapsed_ms, PnxTarget *target) {
  ...
  if (a->phase == PHASE_RUN) {
    ...
    const size_t got = pnx_platform_resource_read(s_resource_ids[size_idx], offset,
                                                   a->buf, READ_LEN);
...
int main(void) {
  ...
  if (!a.has_font) pnx_log("flashbench: font would not load -- nothing to draw");

...
static uint8_t step_tier(uint8_t step)
{
	return (uint8_t)(step / PATH_COUNT);
}
static Path step_path(uint8_t step)
{
	return (Path)(step % PATH_COUNT);
}
...
static void report_results(App* a)
{
	...
	for (int t = 0; t < TIER_COUNT; t++)
	{
		for (int p = 0; p < PATH_COUNT; p++)
			us[p][t] = per_call_us(a->ms[p][t], a->calls[p][t]);
	...
	for (int p = 0; p < PATH_COUNT; p++)
	{
		...
		pnx_format(a->fit[p], sizeof(a->fit[p]), "%s ~%dus + %dus/glyph", PATH_NAME[p],
				   (int)fixed, (int)marginal);
		pnx_log("textbench: FIT %s (from %u/%u glyphs) ~%dus + %dus/glyph", PATH_NAME[p],
				(unsigned)TIER_GLYPHS[0], (unsigned)TIER_GLYPHS[TIER_COUNT - 1], (int)fixed,
				(int)marginal);
	}
}
		...
		switch (a->phase)
		{
			...
			case PHASE_RUN:
				pnx_format(a->status, sizeof(a->status), "%ug %s  %u/%u",
						   (unsigned)TIER_GLYPHS[step_tier(a->step)],
						   PATH_NAME[step_path(a->step)], (unsigned)a->phase_frame + 1,
						   FRAMES_PER_SUBPHASE);
				break;
			case PHASE_DONE:
...
int main(void)
{
	...
	if (!a.has_font1)
		pnx_log("textbench: 1bpp font would not load -- nothing to draw");
	if (!a.has_font2)
		pnx_log("textbench: 2bpp font would not load -- that tier stays 0");

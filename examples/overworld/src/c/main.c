...
static bool enter_scene(Game *g, uint8_t scene, int32_t tx, int32_t ty) {
  ...
  g->sprites[HERO] = (PnxSpriteInstance){
    .x = tx * T + T / 2, .y = ty * T + T,   // feet, centre of the tile
    .sprite = 0, .frame = HERO_STAND,
...
static void try_move(Game *g, int32_t dx, int32_t dy) {
  ...
  if (pnx_map_solid(map, nx, ny)) return;
...
static void frame(void *ctx, uint32_t elapsed_ms, PnxTarget *target) {
  ...
  while (pnx_platform_poll_event(&ev)) {
    ...
    switch (ev.button) {
      case PNX_BUTTON_UP:     g->input_dy = -1; break;
      case PNX_BUTTON_DOWN:   g->input_dy =  1; break;
      case PNX_BUTTON_SELECT: g->input_dx =  1; break;
      default: break;
    }
  }
  ...
  if (st && g->ticks % 50 == 0 && st->frames) {
    pnx_format(g->hud, sizeof(g->hud), "%u.%ufps work %uus",
               (unsigned)(st->fps_x10 / 10), (unsigned)(st->fps_x10 % 10),
               (unsigned)st->work_us);
    if (g->ticks == 50) pnx_diag_flush();
...
int main(void) {
  ...
  if (!pnx_scenes_load(PNX_ASSET_SCENES_SCENES)) {
    ...
    g.ready = enter_scene(&g, PNX_SCENE_OUTDOOR,
                          MAP_OUTDOOR_START_X, MAP_OUTDOOR_START_Y);
  }

...
static void sim_tick(Game *g) {
  ...
  g->bar_x += g->bar_dir * 3;
  if (g->bar_x < 0) { g->bar_x = 0; g->bar_dir = 1; }
  if (g->bar_x > 180) { g->bar_x = 180; g->bar_dir = -1; }
}
...
static void frame(void *ctx, uint32_t elapsed_ms, PnxTarget *target) {
  ...
  if (!g->flushed_startup_logs &&
      pnx_platform_now_ms() - g->started_ms > 3000) {
...
int main(void) {
  ...
  pnx_log("pnx empty: arena %u bytes, touch=%d",
          (unsigned)g.arena.capacity, (int)pnx_platform_has_touch());

...
static void feed_audio(App* a)
{
	...
	if (a->last_state == PNX_AUDIO_PLAYING && st != PNX_AUDIO_PLAYING && a->underruns < 0xFFFF)
	{
		a->underruns++;
	}
...
static void run_case(App* a, uint8_t i)
{
	...
	pnx_log("synth %-11s %4u ns/sample  %u.%02u%% of a core", a->cases[i].name,
			(unsigned)b.ns_per_sample, (unsigned)(b.pct_of_realtime / 100),
			(unsigned)(b.pct_of_realtime % 100));
}
...
static void compose(App* a)
{
	...
	pnx_format(a->line[1], sizeof(a->line[1]), "heap %u B (fx %u)", (unsigned)a->heap_total,
			   (unsigned)a->heap_effects);
	pnx_format(a->line[2], sizeof(a->line[2]), "peak %d/127 hr>>%u%s", (int)a->peak,
			   (unsigned)pnx_synth_headroom(), a->peak > 127 ? " CLIP" : "");
	...
	pnx_format(a->line[5], sizeof(a->line[5]), "%u/%u  %s", (unsigned)(a->showing + 1),
			   (unsigned)a->case_count, cur->name);
	...
	if (a->running)
	{
		...
		if (a->next_case >= a->case_count)
		{
			...
			pnx_log("synth heap %u B (effects %u B)  peak %d/127 at headroom >>%u",
					(unsigned)a->heap_total, (unsigned)a->heap_effects, (int)a->peak,
					(unsigned)pnx_synth_headroom());
		}
	}
...
int main(void)
{
	...
	if (!app.audio_on)
		pnx_log("synth spike: speaker would not open");
	else
	{
		PnxInstrument inst = worst_instrument();
		for (int v = 0; v < PNX_SYNTH_SLOTS; v++)

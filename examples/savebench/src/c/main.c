...
static void log_size_results(App* a)
{
	...
	{
		const uint32_t us =
			a->write_calls[i] ? (a->write_ms[i] * 1000u) / a->write_calls[i] : 0;
		const uint8_t chunks = chunk_count_for(SIZES[i]);
		pnx_format(a->result[i], sizeof(a->result[i]), "%uB (%uc): %ums", (unsigned)SIZES[i],
				   (unsigned)chunks, (unsigned)(us / 1000));
		pnx_log("savebench: %uB (%u chunks) -- %uus/write (%u reps, %u mismatched)",
				(unsigned)SIZES[i], (unsigned)chunks, (unsigned)us, (unsigned)a->write_calls[i],
				(unsigned)a->write_mismatches[i]);
	}
...
static void frame(void* ctx, uint32_t elapsed_ms, PnxTarget* target)
{
	...
	while (pnx_platform_poll_event(&ev))
	{
			...
			(a->phase == PHASE_IDLE || a->phase == PHASE_DONE))
		{
			...
			pnx_diag_flush();
			pnx_log(
				"savebench: run started -- %u sizes x %u reps, then incremental, then armed",
		...
		else if (ev.type == PNX_EVENT_FOCUS_GAINED && a->phase == PHASE_COVERED)
		{
			...
			pnx_format(a->armed_result, sizeof(a->armed_result),
					   "lost->saved %ums, covered %ums", (unsigned)(a->t_saved - a->t_lost),
					   (unsigned)(a->t_gained - a->t_lost));
		...
		case PHASE_WRITE_SIZES:
			{
				...
				for (size_t i = 0; i < bytes; i++)
					s_payload[i] = (uint8_t)(i ^ a->size_index);
				...
				const bool loaded =
					ok && pnx_save_load(TEST_SLOT, s_readback, bytes, SAVE_VERSION, &got);
				...
				if (a->rep >= REPS_PER_SIZE)
				{
					...
					pnx_log("savebench: %uB done -- %uus/write (%u reps)", (unsigned)bytes,
							(unsigned)((a->write_ms[a->size_index] * 1000u) /
									   a->write_calls[a->size_index]),
		...
		case PHASE_INCREMENTAL:
			{
				...
				a->incr_frames++;
				if (frame_ms > a->incr_worst_ms)
					a->incr_worst_ms = frame_ms;
...
int main(void)
{
	...
	if (!a.has_font)
		pnx_log("savebench: font would not load -- nothing to draw");

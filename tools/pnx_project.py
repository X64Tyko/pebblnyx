...
#
# After linking, the size report runs automatically -- see every other example under
# pebblnyx/examples/ for why: the 64KB static-RAM ceiling is a uint16 that fails with an
# unreadable error when exceeded, so the breakdown needs to be in front of you on every
# build, not on request. framework_root assumes this project sits directly under
# pebblnyx/examples/<name>/, two levels below the framework root -- a project scaffolded
# somewhere else on disk will need that adjusted by hand.
...
_COMPRESS_MODE_DEFINE = {
    ...
    'huffman': 'PNX_COMPRESS_MODE=PNX_COMPRESS_HUFFMAN',
}

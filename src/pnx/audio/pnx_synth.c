...
static const uint32_t SEMITONE_Q16[12] = {
  ...
  92682, 98195, 104037, 110228, 116788, 123741
};
...
static void voice_steps(Voice *v, int32_t lfo) {
  for (int oi = 0; oi < PNX_SYNTH_OSCILLATORS; oi++) {
    ...
    if (v->inst.lfo_target == PNX_LFO_PITCH) cents += lfo >> 1;
    ...
    if ((stage) != ENV_OFF) {                                                  \
      ...
      if ((stage) == ENV_ATTACK) {                                             \
        if ((level) >= (1 << 16)) {                                            \
          ...
          if (!(e).decay_ms) { (level) = _t; (rate) = 0; (stage) = ENV_SUSTAIN; } \
        }                                                                      \
      } else if ((stage) == ENV_DECAY) {                                       \
        if ((level) <= _t) { (level) = _t; (rate) = 0; (stage) = ENV_SUSTAIN; }\
      } else if ((stage) == ENV_RELEASE) {                                     \
        if ((level) <= 0) { (level) = 0; (rate) = 0; (stage) = ENV_OFF; }      \
      }                                                                        \
    }                                                                          \
  } while (0)
...
bool pnx_synth_init(uint32_t sample_rate) {
  ...
  for (int i = 0; i < COMBS; i++) line_samples += COMB_LEN[i];
  for (int i = 0; i < ALLPASS; i++) line_samples += ALLPASS_LEN[i];
  ...
  if (!p) {
    pnx_log("synth: %u bytes refused",
            (unsigned)(block_bytes + line_bytes + wave_bytes));
    return false;
  }
  s_effect_bytes = (uint32_t)line_bytes;
  s_total_bytes = (uint32_t)(block_bytes + line_bytes + wave_bytes);

  s_dry      = (int32_t *)p; p += sizeof(int32_t) * BLOCK;
  s_rev_send = (int32_t *)p; p += sizeof(int32_t) * BLOCK;
  s_cho_send = (int32_t *)p; p += sizeof(int32_t) * BLOCK;
  s_block_base = (uint8_t *)s_dry;
  for (int i = 0; i < COMBS; i++) alloc_line(&s_comb[i], COMB_LEN[i], &p);
  for (int i = 0; i < ALLPASS; i++) alloc_line(&s_allpass[i], ALLPASS_LEN[i], &p);
  ...
  for (int m = 0; m < MIPS; m++) {
    ...
    if (s_rate < 16000u) h = (h > 2) ? h / 2 : 1;
    if (h > CYCLE / 2 - 1) h = CYCLE / 2 - 1;
...
uint32_t pnx_synth_bytes(void) { return s_total_bytes; }
uint32_t pnx_synth_effect_bytes(void) { return s_effect_bytes; }

void pnx_synth_set_config(const PnxSynthConfig *cfg) {
  if (cfg) s_cfg = *cfg;
  if (s_cfg.oscillators < 1) s_cfg.oscillators = 1;
...
const PnxSynthConfig *pnx_synth_config(void) { return &s_cfg; }

void pnx_synth_set_headroom(uint8_t shift) { s_headroom = shift > 6 ? 6 : shift; }
uint8_t pnx_synth_headroom(void) { return s_headroom; }

void pnx_synth_set_instrument(uint8_t slot, const PnxInstrument *inst) {
  if (slot >= PNX_SYNTH_SLOTS || !inst) return;
  s_slot[slot] = *inst;
}

void pnx_synth_note_on(uint8_t slot, uint8_t midi_note, uint8_t velocity) {
  if (slot >= PNX_SYNTH_SLOTS || !s_ready) return;
  ...
  v->inst = s_slot[slot];
  if (v->inst.osc_count < 1) v->inst.osc_count = 1;
  v->note = midi_note;
  v->velocity = velocity;
  v->active = true;
  ...
  v->amp_rate = v->inst.amp.attack_ms
      ? env_ramp(v->inst.amp.attack_ms, 1 << 16, s_rate) : (1 << 16);
  v->cut_level = 0;
  v->cut_stage = ENV_ATTACK;
  v->cut_rate = v->inst.cutoff.attack_ms
      ? env_ramp(v->inst.cutoff.attack_ms, 1 << 16, s_rate) : (1 << 16);
  v->lfo_phase = 0;
...
void pnx_synth_all_off(void) {
  for (int i = 0; i < PNX_SYNTH_VOICES; i++) s_voice[i].active = false;
}

uint8_t pnx_synth_active_voices(void) {
  uint8_t n = 0;
  for (int i = 0; i < PNX_SYNTH_VOICES; i++) if (s_voice[i].active) n++;
  return n;
}
...
static int32_t chorus_sample(int32_t in) {
  ...
  if (depth >= s_chorus.len) depth = (uint16_t)(s_chorus.len - 1u);
...
void pnx_synth_render(int16_t *out, uint32_t count) {
  ...
  uint8_t sounding = 0;
  for (int i = 0; i < PNX_SYNTH_VOICES; i++) if (s_voice[i].active) sounding++;
  if (sounding) {
    s_tail = TAIL_BLOCKS;
  } else if (s_tail == 0) {
  ...
  for (uint32_t base = 0; base < count; base += BLOCK) {
    ...
    for (int vi = 0; vi < PNX_SYNTH_VOICES; vi++) {
      ...
      for (; n < n_block; n++) {
        ...
        if (do_filter) {
          ...
          if (s_cfg.lfo && lfo_target == PNX_LFO_CUTOFF) cut += lfo;
          if (cut < 4) cut = 4;
          if (cut > 250) cut = 250;
        ...
        if (first) {
          s_dry[n] = sample;
          if (s_cfg.reverb) s_rev_send[n] = (sample * rev_send) >> 8;
          if (s_cfg.chorus) s_cho_send[n] = (sample * cho_send) >> 8;
        } else {
          s_dry[n] += sample;
          if (rev_send) s_rev_send[n] += (sample * rev_send) >> 8;
          if (cho_send) s_cho_send[n] += (sample * cho_send) >> 8;
        }
      }
      ...
      if (first) {
        for (uint32_t k = n; k < n_block; k++) {
          s_dry[k] = 0;
          if (s_cfg.reverb) s_rev_send[k] = 0;
          if (s_cfg.chorus) s_cho_send[k] = 0;
        }
        first = false;
      }

      // --- written back once per block, not once per sample
      v->amp_level = amp_level; v->amp_rate = amp_rate; v->amp_stage = amp_stage;
      v->cut_level = cut_level; v->cut_rate = cut_rate; v->cut_stage = cut_stage;
      v->lfo_phase = lfo_phase;
      v->pitch_env = pitch_env;
      v->flt_low = flt_low; v->flt_band = flt_band;
      v->mod_count = mod_count;
      for (int oi = 0; oi < PNX_SYNTH_OSCILLATORS; oi++) {
        v->osc[oi].phase = ph[oi];
        v->osc[oi].step = st[oi];
      }
      if (amp_stage == ENV_OFF) v->active = false;
    }
    ...
    if (first) {
      ...
      if (s_cfg.reverb) memset(s_rev_send, 0, sizeof(int32_t) * n_block);
      if (s_cfg.chorus) memset(s_cho_send, 0, sizeof(int32_t) * n_block);
    }

    if (!sounding && s_tail) s_tail--;
    ...
    for (uint32_t n = 0; n < n_block; n++) {
      int32_t wet = 0;
      if (s_cfg.reverb) wet += reverb_sample(s_rev_send[n]);
      if (s_cfg.chorus) wet += chorus_sample(s_cho_send[n]);
...
void pnx_synth_bench(uint32_t chunks, uint32_t count, PnxSynthBench *out) {
  ...
  if (!s_ready || !chunks || !count) return;
  ...
  if (out->samples) {
    ...
    out->ns_per_sample =
        (uint32_t)(((uint64_t)elapsed * 1000000u) / out->samples);
    ...
    out->pct_of_realtime =
        (uint32_t)(((uint64_t)out->ns_per_sample * s_rate) / 100000u);
  }
}

...
static void build_wavetable(void)
{
	...
	for (int i = 0; i < CYCLE; i++)
	{
		...
		const int saw =
			(i < CYCLE / 2) ? (i * 200 / CYCLE) : ((i - CYCLE / 2) * 200 / CYCLE) - 100;
...
static uint32_t output_rate(void)
{
	return (s_format == PNX_AUDIO_16KHZ_16BIT || s_format == PNX_AUDIO_16KHZ_8BIT) ? 16000u
																				   : 8000u;
}
...
{
	if (!s_on || !pcm || samples == 0 || sample_hz == 0)
...
{
	if (voice < PNX_AUDIO_VOICES)
		s_voices[voice].active = false;
}
...
static uint32_t mix(uint32_t count)
{
	...
	for (int v = 0; v < PNX_AUDIO_VOICES; v++)
	{
		...
		for (uint32_t n = 0; n < count; n++)
		{
			...
			if (index >= o->samples)
			{
				if (o->loop_start == PNX_AUDIO_NO_LOOP)
				{
					o->active = false;
					break;
				}
			...
			const int32_t enveloped =
				(o->stage == ENV_OFF) ? sample : ((sample * (o->level >> 8)) >> 8);
	...
	for (uint32_t n = 0; n < count; n++)
	{
		...
		if (v > 127 || v < -128)
		{
			if (s_stats.clipped < 0xFFFFFFFFu)
				s_stats.clipped++;
...
void pnx_audio_update(uint32_t now_ms)
{
	...
	{
		if (s_stats.left_playing < 0xFFFF)
			s_stats.left_playing++;
	}
	...
	if (consumed > s_stats.written)
	{
		...
		if (deficit > s_stats.worst_deficit)
			s_stats.worst_deficit = deficit;
	}
	...
	if (s_carry_bytes > s_carry_head)
	{
		...
		const size_t wrote =
			pnx_platform_audio_write((const uint8_t*)s_mix + s_carry_head, left);
		s_stats.written += (uint32_t)wrote;
		s_carry_head += (uint32_t)wrote;
	...
	if (samples > s_stats.feed_max)
		s_stats.feed_max = (uint16_t)samples;
...
void pnx_audio_set_lowpass(uint16_t cutoff_hz)
{
	...
	if (!cutoff_hz || !s_on)
	{
		s_lp_a = cutoff_hz ? s_lp_a : 0;
		return;
	}

	const uint32_t rate = output_rate();
	if (cutoff_hz * 2u >= rate)
	{
		s_lp_a = 0;
		return;

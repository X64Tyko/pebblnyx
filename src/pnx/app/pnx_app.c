...
void pnx_app_push(const PnxAppOps *ops) {
  ...
  if (was && was->suspend) was->suspend(s_ctx);
  ...
  if (s_depth >= PNX_APP_MAX_DEPTH) {
    ...
    if (was && was->resume) was->resume(s_ctx);
    return;
  }

  s_stack[s_depth++] = ops;
  s_accumulator_ms = 0;
  if (ops->enter) ops->enter(s_ctx);
}
...
void pnx_app_pop(void) {
  ...
  if (was->exit) was->exit(s_ctx);

  s_accumulator_ms = 0;
  const PnxAppOps *now = top();
  if (now && now->resume) now->resume(s_ctx);
}
...
uint8_t pnx_app_depth(void) { return s_depth; }

bool pnx_app_covered(void) { return s_covered; }
...
static void forward_push(PnxEvent ev) {
  ...
  if (s_fwd_count >= FWD_QUEUE_LEN) return;
...
static void pump_events(void) {
  ...
  while (pnx_platform_poll_event(&ev)) {
    if (ev.type == PNX_EVENT_FOCUS_LOST) {
      ...
      if (t && t->suspend) t->suspend(s_ctx);
    } else if (ev.type == PNX_EVENT_FOCUS_GAINED) {
      s_covered = false;
      const PnxAppOps *t = top();
      if (t && t->resume) t->resume(s_ctx);
    } else {
      forward_push(ev);
    }
  }
...
void pnx_app_frame(void *unused_ctx, uint32_t elapsed_ms, PnxTarget *target) {
  ...
  if (!s_covered) {
    ...
    if (s_accumulator_ms > max_ms) s_accumulator_ms = max_ms;

    while (s_accumulator_ms >= (uint32_t)PNX_TICK_MS) {
      s_accumulator_ms -= PNX_TICK_MS;
      if (t->tick) t->tick(s_ctx);
  ...
  if (t->frame) t->frame(s_ctx);
  if (t->draw) t->draw(s_ctx, target);

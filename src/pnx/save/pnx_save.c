...
static bool write_chunk(void)
{
	...
	if (chunk == 0)
	{
		...
		const size_t n =
			s_writer.bytes < PNX_SAVE_CHUNK0_PAYLOAD ? s_writer.bytes : PNX_SAVE_CHUNK0_PAYLOAD;
	...
	{
		pnx_log("save: slot %u chunk %u write failed -- abandoned", (unsigned)s_writer.slot,
				(unsigned)chunk);
	...
	if (s_writer.next_chunk >= s_writer.chunk_count)
		s_writer.active = false;
	return true;
}
...
{
	if (!data || bytes > PNX_SAVE_MAX_PAYLOAD)
		return false;
...
{
	if (!pnx_save_begin(slot, data, bytes, version))
		return false;
...
bool pnx_save_peek_version(PnxSaveSlot slot, uint8_t* out_version)
{
	...
	size_t got = 0;
	if (!pnx_platform_persist_read(key_for(slot, 0), buf, sizeof(buf), &got))
		return false;
	if (got < PNX_SAVE_HEADER_BYTES || buf[0] != 'P' || buf[1] != 'S')
		return false;
	if (out_version)
		*out_version = buf[2];
	return true;
				   ...
				   size_t* out_bytes)
{
	...
	size_t got0 = 0;
	if (!pnx_platform_persist_read(key_for(slot, 0), buf0, sizeof(buf0), &got0))
		return false;
	...
	if (chunk_count == 0 || chunk_count > PNX_SAVE_CHUNKS_PER_SLOT)
		return false;
	if (payload_bytes > PNX_SAVE_MAX_PAYLOAD || payload_bytes > max_bytes)
		return false;

	const size_t n0 =
		payload_bytes < PNX_SAVE_CHUNK0_PAYLOAD ? payload_bytes : PNX_SAVE_CHUNK0_PAYLOAD;
...
bool pnx_save_delete(PnxSaveSlot slot)
{
	...
	{
		if (pnx_platform_persist_delete(key_for(slot, c)))
			any = true;
	}
	return any;
}

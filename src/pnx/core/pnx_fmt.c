...
static void emit_pad(Sink *s, char c, int count) {
  for (int i = 0; i < count; i++) emit(s, c);
}
...
static void emit_str(Sink *s, const char *str, int width, bool left_align, char pad) {
  ...
  if (!left_align) emit_pad(s, pad, padding);
  for (int i = 0; i < len; i++) emit(s, str[i]);
  if (left_align) emit_pad(s, ' ', padding);
}
...
int pnx_vformat(char *buf, size_t size, const char *fmt, va_list ap) {
  ...
  for (const char *p = fmt; *p != '\0'; p++) {
    ...
    p++;
    if (*p == '\0') break;      // trailing '%' with nothing after it
    ...
    if (!left_align && pad == ' ') emit_pad(&s, ' ', padding);
    if (negative) emit(&s, '-');
    if (!left_align && pad == '0') emit_pad(&s, '0', padding);

    for (int i = 0; digits[i] != '\0'; i++) emit(&s, digits[i]);

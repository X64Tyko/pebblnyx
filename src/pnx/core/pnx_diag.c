...
void pnx_diag_flush(void)
{
	...
	{
		char note[64];
		pnx_format(note, sizeof(note), "--- %u earlier lines dropped ---", (unsigned)s_dropped);
...
void pnx_diag_frame(uint32_t frame_ms, uint32_t work_ms)
{
	...
	if (work_ms > s_window_worst_ms)
		s_window_worst_ms = work_ms;

	if (s_window_ms < STATS_WINDOW_MS)
		return;

...
void pnx_input_init(uint8_t orientation) {
  s_orientation = orientation < PNX_ORIENT_COUNT ? orientation
...
void pnx_input_event(const PnxEvent *ev) {
  if (!ev || ev->button >= PNX_BUTTON_COUNT) return;
  ...
  if (ev->type == PNX_EVENT_BUTTON_DOWN) {
    ...
    if (!s_held[ev->button]) s_since[ev->button] = ev->time_ms;
...
uint32_t pnx_input_held_ms(PnxButton button, uint32_t now_ms) {
  if (button >= PNX_BUTTON_COUNT || !s_held[button]) return 0;
...
PnxButton pnx_input_cluster(uint8_t pos) {
  if (pos >= PNX_CLUSTER_SIZE) return PNX_BUTTON_COUNT;

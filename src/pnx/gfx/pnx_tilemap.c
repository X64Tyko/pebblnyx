...
static void draw_worldtile(const PnxMap *map, const PnxWorldTile *wt,
  ...
  for (int32_t ty = y0; ty <= y1; ty++) {
    ...
    for (int32_t tx = x0; tx <= x1; tx++) {
      ...
      if (!atlas) continue;   // its slot was evicted; the WorldTile pin makes this unreachable
      ...
      if (pnx_atlas_is_metatiled(atlas)) {
        ...
        pnx_blit_4bpp(target, pnx_atlas_tile(atlas, (uint8_t)local), pal,
...
void pnx_tilemap_draw(const PnxMap *map, PnxTarget *target, const PnxCamera *camera) {
  if (!map || !map->slots || map->tile_px == 0) return;
  ...
  if (last_tx >= map->w) last_tx = map->w - 1;
  if (last_ty >= map->h) last_ty = map->h - 1;
  if (last_tx < tx0 || last_ty < ty0) return;
  ...
  for (int32_t wy = ty0 >> shift; wy <= (last_ty >> shift); wy++) {
    for (int32_t wx = tx0 >> shift; wx <= (last_tx >> shift); wx++) {
      ...
      if (slot == PNX_MAP_NO_SLOT) continue;

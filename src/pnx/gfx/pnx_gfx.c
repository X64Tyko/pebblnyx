...
static void span_4bpp(uint8_t *row_base, int32_t x, const uint8_t *line,
  ...
  if (i & 1) {
    ...
    if (v != PNX_PALETTE_TRANSPARENT) dst[i] = pal[v];
    i++;
  }
  for (; i + 1 < i1; i += 2) {
    ...
    if (hi != PNX_PALETTE_TRANSPARENT) dst[i] = pal[hi];
    if (lo != PNX_PALETTE_TRANSPARENT) dst[i + 1] = pal[lo];
  }
  if (i < i1) {
...
void pnx_blit_metatile_with(PnxTarget *t, const PnxAtlas *atlas, uint8_t tile,
  ...
  if (!palette || !atlas->metatiles) return;
  ...
  for (int32_t j = j0; j < j1; j++) {
    ...
    for (int32_t k = 0; k < 2; k++) {
      const uint8_t *line = atlas->pixels
                          + (uint32_t)pair[k] * atlas->sub_bytes
                          + qj * sub_stride;
      span_4bpp(row.data, x + k * half, line, palette->entries,
                half, row.min_x, row.max_x);
    }
  }
}
...
void pnx_blit_4bpp(PnxTarget *t, const uint8_t *src, const PnxPalette *palette,
  ...
  for (int32_t j = j0; j < j1; j++) {
    ...
    int32_t i0 = 0, i1 = w;
    if (x + i0 < row.min_x) i0 = row.min_x - x;

...
// PnxAnimState/pnx_anim_play/pnx_anim_frame moved to core/pnx_anim.h (tile animation
// needed the same playback primitive without depending on PNX_USE_SPRITES) -- re-exported
// via the #include above, so every existing call site (PnxAnimState state;
// pnx_anim_play(...); pnx_anim_frame(...)) keeps compiling unchanged against this header.
